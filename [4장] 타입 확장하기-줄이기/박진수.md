...
- 자바스크립트의 동작 방식으로 인해 null과 배열 타입 등이 object 타입으로 판매되는 등 복잡한 타입을 검증하기에는 한계가 있기 때문이다.
...
export type NoticeDialogProps = BasicNoticeDialogProps | NoticeDialogWithCookieProps; // 책에는 여기에 코드 오류 있음
```
- 이렇게 코드가 선언되었다고 했을 때 in 연산자를 사용하는 방법은 다음과 같다.
```tsx
...
- 타입스크립트에서는 객체 타입에 속성이 존재하는지를 검사하므로 타입 가드의 역할을 한다.
